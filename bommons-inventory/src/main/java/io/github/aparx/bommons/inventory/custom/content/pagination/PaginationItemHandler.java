...
public final class PaginationItemHandler {
  ...
  private static final InventoryItem DEFAULT_PREVIOUS_PAGE = InventoryItemFactory.ofCancelled(
      ItemStackBuilders.create(Material.ARROW).name("Previous").build());

  private static final InventoryItem DEFAULT_NEXT_PAGE = InventoryItemFactory.ofCancelled(
      ItemStackBuilders.create(Material.ARROW).name("Next").build());

  private static final InventoryItem DEFAULT_PLACEHOLDER = InventoryItemFactory.ofCancelled(
